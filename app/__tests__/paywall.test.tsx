...
describe("Paywall", () => {
  ...
  // The paywall's shape: a muted "before" card describing the free game,
  // connected to an accent "after" card listing what the purchase unlocks.
  // Asserting both renders is what would catch a regression back to a plain
  // list -- the structure this app is required to look different from.
  it("renders the before state and the after state as two distinct cards", async () => {
    const { getByText } = await renderWithProviders(<Paywall />);
    expect(getByText(t("paywallBeforeEyebrow"))).toBeTruthy();
    expect(getByText(t("paywallBeforeDesc"))).toBeTruthy();
    expect(getByText(t("paywallAfterEyebrow"))).toBeTruthy();
  });
